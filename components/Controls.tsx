  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 z-50 pointer-events-none">
      ...
      <div className="flex items-center gap-2 pointer-events-auto bg-white/90 backdrop-blur-md shadow-xl rounded-full p-1 border border-gray-100">
        <button
            onClick={() => setTool('pan')}
            className={`px-4 py-2 rounded-full font-medium text-sm transition-all ${tool === 'pan' ? 'bg-black text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}
        >
        ...
        <button
            ...
            className={`px-4 py-2 rounded-full font-medium text-sm transition-all ${tool === 'stamp' ? 'bg-rose-500 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}
        >
            <span className="flex items-center gap-2">
